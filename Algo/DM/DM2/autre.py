...
def ladder(G, start, end):
    ...
    status = True
    index = 0
    while index < len(G.labels) and status:
        if G.labels[index] == start:
            status = False
        else:
            index += 1
    src = index
